...
def random_velocity_assignment(complete_velocity):
    ...
    # FALSE: draw random numbers between -0.5*math.pi and 0.5*math.pi and 0 and 2 times math.pi (spherical coordinate system) for dividing velocity into the three coordinates components
...
# simple function that calculates maximum probability of a Maxwell Boltzmann distribution for a veloctity range, an atom mass and a temperature
#@profile
@jit(cache=True)
def calculate_p_max(n, v_min, v_max, atom_mass, temperature):
    ...
    for i in range(0, n):
        ...
        # if probability of the drawn velocity inserted into the Maxwell Boltzmann distribution is higher than the highest probability this probability is set to be the highest one
